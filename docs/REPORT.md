...
한 문장으로 줄이면, **이 프로젝트의 효과는 AI가 코드를 빨리 만든 데 있지 않고, 바이브코딩의 비결정성과 과장을 요구분석·테스트·문서 검증·형상관리로 드러내고 통제한 데 있습니다.** 검출은 LLM을 한 번도 호출하지 않는 결정론적 메트릭으로 하고(검출 레이어의 LLM 호출 0), 요구는 REQ→UC→코드→테스트로 추적되며, 40개 테스트가 커밋마다 자동으로 실행됩니다. 효과의 실체는 빠른 생산이 아니라 이렇게 과정을 측정 가능하게 만든 데 있습니다.
...
아래 캡처들은 도구 기능 자랑이 아니라, 비결정성을 통제하는 절차가 실제로 도는 증거입니다. 검출은 단독으로 결정론적으로 돕니다(같은 코드 → 같은 결과).
...
아래 표가 이 보고서의 핵심입니다. 요구분석부터 품질 관리까지 각 단계에서 프로세스가 무엇을 통제했고 어떤 교훈을 남겼는지를 한 줄씩 보입니다. 모든 단계를 관통한 원리는 **확률적인 부분과 결정론적인 부분을 가르고, 그 경계를 문서로 남기는 것**이었습니다.
...
동시에 프로세스가 모든 것을 막지는 못했습니다. 12일을 들여 결정론적 검출은 두 지표(SRP·OCP)에 그쳤습니다. 테스트는 40개를 갖췄지만 검출 코어에 집중됐고, 웹 대시보드는 자동화 테스트 없이 수동 확인에 머물렀으며 통합 테스트도 핵심 흐름 하나만 덮습니다. 문서도 한동안 코드와 어긋났습니다. 프로세스는 결함을 0으로 만들지는 못하고, 다만 결함을 더 일찍 더 자주 드러냅니다. 산출물의 위치도 분명합니다. 운영·배포·보안을 갖추지 못했으므로 이것은 prototype이지 MVP가 아닙니다.
...
다시 말하지만, 이 프로젝트의 효과는 AI가 코드를 빨리 만든 데 있지 않습니다. 바이브코딩의 비결정성과 과장을 요구분석·테스트·문서 검증·형상관리로 드러내고 통제한 데 있습니다. 바이브코딩으로 빠르게 만든 프로토타입은 빙산의 일각이고, 요구분석부터 품질 관리까지 프로세스를 한 단계씩 입혔을 때 비로소 결과물을 신뢰할 수 있었습니다.

소프트웨어가 복잡하고 비가시적이라는 위기의 본질은 LLM 시대에도 그대로이고, 오히려 확률적 생성이 비결정성을 하나 더 얹었습니다. 그래서 답도 새 유행보다 오래된 원리, 곧 추측을 검증으로 확인하고 확률적 출력 위에 결정론을 받치며 같은 기록을 공유하는 데 있었습니다. 빠르게 만드는 것은 AI가 맡고, 그 결과를 믿을 수 있게 만드는 것은 여전히 프로세스의 몫이라는 것이 이 프로젝트가 남긴 결론입니다.
