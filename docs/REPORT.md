...
프로세스를 입히자 바이브코딩의 결과물을 "그러려니" 쓰는 대신 의심하고 고칠 수 있었습니다. 효과는 세 장면으로 압축됩니다. 요구분석에서 신뢰성 요구를 먼저 고정한 덕분에 흔들리는 LLM 점수를 "정상"이 아닌 요구 위반으로 읽어 설계를 피벗했고, 테스트·문서 검증 단계가 god class 오검출 버그와 문서의 과장을 잡아냈으며, 범위를 줄여 마감 가능한 핵심을 지켰습니다.

여기서 말하는 "효과"는 "AI가 코드를 빨리 짜줬다"가 아니라 **과정을 측정 가능하게 만든 것**입니다. 검출은 LLM을 한 번도 호출하지 않는 결정론적 메트릭으로 하고(검출 층 LLM 호출 0), 요구는 REQ→UC→코드→테스트로 추적되며, 40개 테스트가 커밋마다 자동으로 돕니다.

## 2. 배경: 소프트웨어 위기와 왜 프로세스인가

소프트웨어는 복잡하고 비가시적이라 추측에 기대기 쉽고, 그래서 체계와 검증을 요구합니다. 바이브코딩은 이 위기를 새 형태로 되살립니다. 요구·설계·테스트·문서가 뒤로 밀리는 데다, LLM은 출력이 확률적이라 같은 입력에 다른 결과를 냅니다.

근거는 분명합니다. AI 생성 코드는 직접 작성보다 버그 약 1.7배·보안 취약점 약 2배로 보고되고(CodeRabbit 2025), 470개 PR을 분석한 후속 연구는 보안 취약점 2.74배를 보고합니다(2025.12). AI 리뷰 도구의 실제 버그 검출 정확도는 46%에 그쳐, AI가 AI 산출물을 끝까지 검증하지 못함을 보여줍니다. "돌아간다"와 "안전하게 쓸 수 있다"의 차이를 메우는 것이 프로세스이고, NaN-SE는 코드를 만드는 과정에 그 절차를 끼워넣는 시도입니다.

## 3. 무엇을 만들었나

NaN-SE는 AI가 만든 코드의 응집도·복잡도 위반을 결정론적으로 검출하고, 그 위반을 LLM이 학습 카드로 설명하며, 채택·거절은 사람이 하는 도구입니다. 핵심은 **검출(결정론)과 설명(LLM)을 다른 층으로 가른 것**입니다.
...
바이브코딩으로 빠르게 만든 프로토타입은 빙산의 일각입니다. 요구분석부터 품질 관리까지 프로세스를 한 단계씩 입혔을 때 비로소 결과물을 신뢰할 수 있습니다. 소프트웨어가 복잡하고 비가시적이라는 위기의 본질은 LLM 시대에도 그대로이고, 오히려 확률적 생성이 비결정성을 하나 더 얹었습니다. 그래서 답도 새 유행보다 오래된 원리, 곧 추측을 검증으로 확인하고 흔들리는 확률 위에 결정론을 받치며 같은 기록을 공유하는 데 있었습니다. 이 프로젝트는 그 원리를 바이브코딩이라는 새 맥락에 다시 적용해 본 기록입니다.

## 참고문헌

각 출처를 어디에 썼는지 함께 적습니다.

- McCabe, "A Complexity Measure" (1976): 순환복잡도 정의·임계 10의 근거 (METRICS 2)
- Hitz & Montazeri, LCOM4 (1995): 연결 요소 기반 응집도 정의 (METRICS 1)
- Shepperd, "A critique of cyclomatic complexity" (1988): 지표 한계의 비판적 근거 (METRICS 3)
- "Are We SOLID Yet?": LLM의 SOLID 점수 매기기 불안정성, 피벗 근거 (2·4장)
- CodeRabbit (2025): AI 코드 결함·검출 정확도 통계 (2장)
- AI 협업 코드 결함 분석, 470 PR (2025.12): 보안 취약점 2.74배 (2장)
- "Vibe Coding in Practice" (ICSE 2026 SEIP): 통제 중심 실천의 현장 근거 (2장)
...
**효과의 정의를 다시.** 제품 관점에서도 NaN-SE의 가치는 "검증을 대신 해준다"가 아니라 "사람이 귀찮아서 안 하던 검증의 비용을 낮춰 실제로 하게 만든다"에 있습니다. 검출이 좁아도 흔들리지 않는 두 지표를 고른 것, 위반을 점수가 아니라 학습 카드로 풀어 사용자가 원칙을 익히게 한 것이 그 방향의 선택입니다.
