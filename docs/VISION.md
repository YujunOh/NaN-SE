...
NaN-SE는 바이브코딩 시대의 TCP를 표방한다. 불안정한 AI 코딩 위에 SW공학 신뢰성을 얹는 검증·제약 프로토콜로 설계했다. AI가 만든 SW공학 위반을 단순히 차단하지 않고 학습 카드로 바꿔서, 사용자가 원칙을 자연스럽게 익히게 만든다.
...
기능은 분명히 동작한다. 다만 여러 사람이 서로 한 번도 협의한 적 없이 각자 짠 듯한 코드베이스가 만들어진다. 구조적 결함은 통합 시점에야 드러나고, 매 커밋마다 기술부채가 쌓인다. 한 CTO의 표현을 빌리면 평소엔 멀쩡하게 동작하다가 어느 순간 통째로 무너지는 코드다.

### "공장 컨베이어벨트"

손길이 거의 닿지 않은 채 라인 끝에서 쏟아져 나오는 양산형 제품이다. 빠르고 많이 나오지만 출하 전 검수가 빠지면 결국 반품·환불·재작업 비용으로 돌아온다. 바이브코딩의 산출물도 흐름이 비슷하다. 빠르게 생산되고 빠르게 main에 들어가고 며칠 뒤 디버깅 비용으로 회수된다.
...
단순한 도구 사용 미숙으로 보기 어렵고, 소프트웨어 위기가 새 형태로 돌아온 것에 가깝다.
...
1. **흐름을 막는다**: 바이브코딩을 금지하거나 교육·강제한다. 비현실적이고 효과도 제한적이다.
2. **흐름 안에 원칙을 끼워넣는다**: SW공학 원칙을 바이브코딩 흐름에 자연스럽게 끼워서 품질을 보장한다.
...
이 prototype에서 출발해 진짜 product로 가는 길은 길지만, 시작점은 분명하다.
