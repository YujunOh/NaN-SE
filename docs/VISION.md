...
글 설명만으로는 "그래서 뭐가 돌아간다는 건지" 잡히지 않을 수 있다. 실제 CLI 출력 형식으로 하루 흐름을 그려본다. 단, 5.1(Stage 요구사항 누락 검출)은 미구현 모듈의 가상 시연이고, 실제로 돌아가는 것은 5.2의 검출·학습 카드 흐름(`nanse analyze` / `learn` / `review`)이다.
