...
NaN-SE는 Claude CLI(Claude Code)로 개발했습니다. 같은 도구로 자기 결과를 검증하면 편향되므로, 마감 전에 다른 AI 코딩 에이전트인 opencode에 저장소를 그대로 넘겨 독립적으로 다시 확인했습니다. 아래 프롬프트를 opencode 세션에 그대로 넣고 돌렸습니다.
