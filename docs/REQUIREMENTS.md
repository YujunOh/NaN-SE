...
페인포인트에서 기능 요구를 뽑고 우선순위와 유스케이스를 붙였다. REQ-01~04는 구현했고 `nanse trace`가 코드·테스트 존재로 추적한다.

| REQ | 기능 요구 | 우선순위 | UC | 상태 |
|---|---|---|---|---|
| REQ-01 | AI 코드의 SRP·응집도 위반을 결정론적으로 검출 | 높음 | UC-03 | 구현 |
| REQ-02 | 확정된 위반을 학습 카드로 설명 (점수 매기기 없음) | 높음 | UC-04 | 구현 |
| REQ-03 | 학습 카드를 사람이 검수·채택/거절·저장 | 높음 | UC-05 | 구현 |
| REQ-04 | 요구가 코드·테스트로 추적되는지 존재 검증 | 중간 | UC-trace | 구현 |
| REQ-05 | SDLC 단계 누락 검출·제안 (차단 없음) | 낮음 | UC-01·02 | 설계만 |
| REQ-06 | 세션 종료 시 단계별 진행 리포트 | 낮음 | UC-06 | 설계만 |
