...
| 11 | 06-06 | dogfooding + 보고서 초안 | NaN-SE를 다른 작업에 적용 + 보고서 초안 | 4 | 97% |
...
gantt
    ...
    착수                :2026-05-27, 1d
    요구분석             :2026-05-28, 1d
    아키텍처 설계         :2026-05-29, 1d
    section 구현 (Track 1)
    메트릭 검출(LCOM4·radon) :2026-05-30, 2d
    피벗 검출/설명 분리      :crit, 2026-05-31, 1d
    학습 카드 파이프라인      :2026-05-31, 2d
    검수 CLI               :2026-06-02, 1d
    웹 대시보드             :2026-06-01, 2d
    section 점검·보고서
    문서 정직성 점검·정정     :2026-06-06, 1d
    보고서·다이어그램·테스트가이드 :2026-06-06, 2d
    제출 준비(PDF·캡처)      :2026-06-07, 1d
    section 마감
    마감일                 :milestone, 2026-06-08, 0d
```

위는 계획이다. 실제 진행은 커밋 이력과 docs/DISCUSSION_LOG.md에 남아 있고, 06-03~06-05에 기말 다른 과목과 겹쳐 손을 뗐다가 마감 주에 점검으로 복귀한 deviation도 그대로 드러난다. 계획과 실제의 차이를 감추지 않는 것이 일정 관리의 일부다.
...
1. **통합 단계 지연** - 4 트랙이 Day 7에 모이는데 인터페이스 어긋나면 통합 시간 부족. 완충: Day 3에 인터페이스 명세 확정 + Day 6 저녁에 1차 통합 PoC 시도로 호환성 확인. Day 7 통합 합류가 실제로 가능할지는 미지수다 (Brooks 법칙이 1인 + 다중 세션에도 적용된다면 폭발 가능)
...
4. **Brooks 법칙 자체** - 1인 + 다중 세션 운용이 "인력 추가"인지 "단일 인력의 멀티태스킹"인지 모호한 상황. 후자라면 인지 부하만 늘고 통합 비용 폭증 가능. 완충: 모든 세션이 Day 4에 동시 출발, Day 7 이후 신규 트랙 추가 금지. 이 risk가 실제로 터지면 그 자체가 회고 자료가 된다
